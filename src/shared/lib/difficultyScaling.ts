...
/* אורך משפט מרבי מומלץ (מילים) — **לוולידציה תוכנתית רכה בלבד**, לא מוזרק לפרומפט
   (הזרקתו כובלת את המודל לעברית טלגרפית; הפרומפט מקבל הוראת-מטרה במקום). */
...
/* הוראת-מטרה לרמת הניסוח (1-10) — מתארת את **המטרה** (עברית תקנית וברורה לרמה X)
   ולא טכניקה (אורך משפט/מילים אסורות), כי הוראת-מטרה + דוגמה מפיקה עברית טבעית
   ואיכותית יותר מהוראה מכנית. תקניות היא דרישת-על בכל הרמות. */
export function narrativeStyleSpec(level: number): string {
  const l = clampLevel(level)
  if (l <= 3)
    return `כתוב עברית **פשוטה, תקנית וברורה** שילד בכיתות הנמוכות יבין בקלות. משפטים קצרים אך **שלמים וזורמים** (לא טלגרפיים ולא קטועים), מילים מוכרות, רעיון אחד בכל פעם, בהיר וישיר. דוגמת טון: "הזקן קם והוביל אתכם אל כד חרס ישן מאחורי הספסל. הוא לחש שבתוכו מסתתרת תוכנית סודית."`
  if (l <= 6)
    return `כתוב עברית **תקנית וזורמת** לחטיבת ביניים — משפטים בינוניים, אוצר מילים סטנדרטי, מותר מונח תוכן עם הקשר שמסביר אותו. טבעי וקריא. דוגמת טון: "הזקן קם ופנה אל כד חרס עתיק שהוסתר מאחורי הספסל, ובקול נרגש סיפר שבתוכו טמונה תוכנית סודית שתכריע את גורל המערכה."`
  return `כתוב עברית **עשירה, מדויקת ובהירה** — תחביר שמשרת בהירות, לא מתפתל לראווה; אוצר מילים תחומי ומדויק, לא ארכאי. **הקושי חי בעומק הרעיון, לא בקישוט לשוני** — אם אפשר לפשט את הניסוח בלי לאבד את האתגר המחשבתי, פשט. דוגמת טון: "הזקן ניגש אל כד החרס שמאחורי הספסל וחשף בקולו הרוטט שבתוכו טמונה התוכנית שתכריע את גורל המערכה כולה."`
}
