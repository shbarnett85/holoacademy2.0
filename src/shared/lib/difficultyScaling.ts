...
/* השהיה **לתו** — ולכן מספר גדול יותר = הקלדה איטית יותר. הקצוות הם ה-baseline
   היחיד: כל הרמות שביניהן נגזרות מהם ב-lerp ומתכווננות איתם אוטומטית.
   כוונון אחרון: חצי מהמקור (45→90) ואז החשה של 15% (90→78, 24→21). */
export const TYPING_MS_SLOW = 78 /* רמה 1 — האיטי ביותר */
export const TYPING_MS_FAST = 21 /* רמה 20 — המהיר ביותר */
...
export function narrativeStyleSpec(level: number): string {
  ...
  if (l <= 7)
    return `כתוב עברית **תקנית וזורמת** לכיתות ב'–ג'. משפטים קצרים-בינוניים, אוצר מילים יסודי ויומיומי (לעולם לא מילים ספרותיות — "הסתכל" לא "הביט"), מותר מונח אחד עם הסבר קצר. טבעי וברור. דוגמת טון: "הזקן קם וניגש אל כד חרס עתיק שהסתתר מאחורי הספסל. בקול נרגש הוא סיפר שבתוכו טמונה תוכנית סודית."`
  if (l <= 10)
    return `כתוב עברית **תקנית וזורמת** לכיתות ד'–ו'. משפטים בינוניים, אוצר מילים יומיומי שילד בגיל הזה משתמש בו בעצמו — לא מילים ספרותיות ("שקטה" לא "שוקטת", "הסתכל" לא "הביט", "התחיל" לא "החל", "עמד" לא "ניצב"). מותרים מונחי תוכן עם הקשר. קריא וטבעי. דוגמת טון: "הזקן ניגש אל כד החרס שמאחורי הספסל, ובקול נרגש סיפר שבתוכו מסתתרת תוכנית סודית שיכולה להכריע את הקרב."`
  if (l <= 13)
    return `כתוב עברית **תקנית, עשירה אך עכשווית וחיה** לחטיבת הביניים. משפטים מורכבים יותר ואוצר מילים רחב, מותרים מונחים מקצועיים בהקשר — אבל **שפה של היום, לא רגיסטר ספרותי**: גם מילה ספרותית תקינה נפסלת כשיש לה חלופה יומיומית ("הסתכל" לא "הביט", "עמד" לא "ניצב", "האלה" לא "הללו"). העושר בא מהתוכן והמתח, לא מהמילון. דוגמת טון (מורכב אך עכשווי): "במרכז הזירה עמד הנאשם, ושלוש עדויות סותרות ריחפו באוויר. השופט ידע שכל החלטה כאן תשנה את חייו של מישהו."`
  return `כתוב עברית **עשירה, מדויקת, בהירה — ועכשווית** לתיכון ומעלה. הקושי חי ב**עומק הרעיון**, לא במילון ולא בקישוט לשוני. מילה יומיומית עדיפה על ספרותית כשאין ביניהן הבדל משמעות ("הסתכל" ולא "הביט").
