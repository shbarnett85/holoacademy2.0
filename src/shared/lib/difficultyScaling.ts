...
export function narrativeStyleSpec(level: number): string {
  ...
  if (l <= 13)
    return `כתוב עברית **תקנית, עשירה וזורמת** לחטיבת הביניים. משפטים מורכבים יותר, אוצר מילים רחב, מותרים מונחים מקצועיים בהקשר. ברור ולא מסורבל. דוגמת טון: "הזקן ניגש אל כד החרס הנסתר מאחורי הספסל, ובקולו הנרגש חשף שבתוכו טמונה התוכנית שתכריע את גורל המערכה כולה."`
  return `כתוב עברית **עשירה, מדויקת ובהירה** לתיכון ומעלה — תחביר שמשרת בהירות, לא מתפתל לראווה; אוצר מילים תחומי ומדויק, לא ארכאי. **הקושי חי בעומק הרעיון, לא בקישוט לשוני** — אם אפשר לפשט את הניסוח בלי לאבד את האתגר המחשבתי, פשט. דוגמת טון: "הזקן ניגש אל כד החרס שמאחורי הספסל וחשף בקולו הרוטט שבתוכו טמונה התוכנית שתכריע את גורל המערכה כולה."`
}
