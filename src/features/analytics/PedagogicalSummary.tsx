...
h1{font-size:18pt;color:var(--t45)}.meta{color:var(--t46);font-size:10pt;margin-bottom:18pt}
...
export default function PedagogicalSummary({ scope, id, title }: { scope: Scope; id: string; title?: string }) {
  ...
  if (forbidden) return (
    <div style={{ ...glass, padding: '12px 16px', fontSize: 12.5, color: 'var(--t6)', display: 'flex', alignItems: 'center', gap: 7 }}><DrHoloEmblem size={16} /> {forbidden}</div>
  )
  ...
  const btn = (extra: React.CSSProperties = {}): React.CSSProperties => ({ fontSize: 12, fontWeight: 700, padding: '7px 14px', borderRadius: 9, cursor: 'pointer', background: 'var(--t47)', border: '1px solid var(--t48)', color: 'var(--t49)', ...extra })

  return (
    <div style={{ ...glass, padding: 20, borderColor: 'var(--t50)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
          <span style={{ display: 'inline-flex' }}><DrHoloEmblem size={20} /></span>
          <div>
            <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--t49)' }}>סיכום פדגוגי</div>
            <div style={{ ...micro, fontSize: 8.5, color: 'var(--t51)' }}>נכתב ע״י ד״ר הולו · AI{edited ? ' · נערך ע״י המורה' : ''}</div>
          </div>
        </div>
        {summary && !editing && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button onClick={() => generate(true)} disabled={generating} style={btn(generating ? { color: 'var(--t52)', cursor: 'default' } : {})}>{generating ? '✦ כותב…' : '↻ צור מחדש'}</button>
          </div>
        )}
      </div>

      {error && <p style={{ color: 'var(--t18)', fontSize: 13, marginTop: 12 }}>⚠️ {error}</p>}

      {!summary && !generating && (
        <div style={{ marginTop: 14, textAlign: 'center' }}>
          <p style={{ fontSize: 13, color: 'var(--t27)', marginBottom: 12 }}>ד״ר הולו יקרא את הנתונים ויכתוב סיכום פדגוגי בשפה אנושית — תצפיות מבוססות-נתונים, לא אבחנות.</p>
          <button onClick={() => generate(false)}
            style={{ fontSize: 14, fontWeight: 800, color: 'var(--t53)', padding: '11px 26px', borderRadius: 11, cursor: 'pointer', background: 'linear-gradient(135deg, var(--t54), var(--t55))', border: 'none', boxShadow: '0 0 20px var(--t56)' }}>
            ✦ צור סיכום פדגוגי
          </button>
        </div>
      )}
      ...
      {generating && !summary && (
        <p style={{ marginTop: 14, fontSize: 13, color: 'var(--t49)', textAlign: 'center' }}>✦ ד״ר הולו כותב את הסיכום…</p>
      )}

      {summary && editing && (
        <div style={{ marginTop: 14 }}>
          <textarea value={draft} onChange={(e) => setDraft(e.target.value)} rows={9}
            style={{ width: '100%', resize: 'vertical', background: 'var(--t57)', border: '1px solid var(--t48)', borderRadius: 10, color: 'var(--t58)', fontSize: 14, lineHeight: 1.7, fontFamily: 'var(--font-display)', padding: '12px 14px', outline: 'none' }} />
          <div style={{ display: 'flex', gap: 8, marginTop: 10, justifyContent: 'flex-end' }}>
            <button onClick={() => setEditing(false)} style={btn({ color: 'var(--t27)', background: 'transparent', border: '1px solid var(--t59)' })}>ביטול</button>
            <button onClick={saveEdit} disabled={saving} style={{ ...btn({ color: 'var(--t53)', background: 'linear-gradient(135deg, var(--t54), var(--t55))', border: 'none' }) }}>{saving ? 'שומר…' : 'שמור עריכה'}</button>
          </div>
        </div>
      )}
      ...
      {summary && !editing && (
        <>
          <p style={{ marginTop: 14, fontSize: 14.5, lineHeight: 1.7, color: 'var(--t58)', whiteSpace: 'pre-wrap' }}>{text}</p>
          <div style={{ ...micro, fontSize: 8.5, marginTop: 14, color: 'var(--t60)', display: 'flex', gap: 12, flexWrap: 'wrap' }}>
