...
async function runFactCheck(gameData: GameData, sceneIds?: string[]): Promise<{ ok: boolean; errors: FactError[] }> {
  ...
  const instruction = `אתה בודק עובדות **ותקניות-לשון** בכלי חינוכי לילדי בית ספר יסודי. עבור על התוכן הבא וזהה שבעה סוגי בעיות (בעברית עכשווית, לא סגנון ספרותי):
