...
async function callHaiku(messages: { role: 'user' | 'assistant'; content: string }[], maxTokens: number, system?: string): Promise<string> {
...
async function runFactCheck(gameData: GameData, sceneIds?: string[]): Promise<{ ok: boolean; errors: FactError[] }> {
  ...
  const instruction = `אתה בודק עובדות **ותקניות-לשון** בכלי חינוכי. עבור על התוכן הבא וזהה חמישה סוגי בעיות:
...
5. **אי-התאמת מין/מספר דקדוקית** — חוסר התאמה בין שם עצם לבין הפועל/התואר/מילת-השאלה המתייחסים אליו: (א) **מין**: שם עצם נקבה עם פועל/תואר זכר או להפך (למשל "כמה שוקל הגלקסיה" → "שוקלת"; "איזו אור" → "איזה"; "המים זורמת" → "זורמים"). (ב) **מספר**: יחיד מול רבים (למשל "הילדים הולך" → "הולכים"). זהו דפוס דקדוקי ברור — דגל אך ורק אי-התאמה **חד-משמעית ובטוחה**, והצע את הצורה המתאימה. אל תדגל מקרים שנויים-במחלוקת (שם עצם דו-מיני, צורת רבים חריגה).
אל תדגל סגנון, ניסוח ספרותי, העדפה אישית או מילים נדירות-אך-תקינות — רק שגיאות עובדתיות, ביטויים מדעיים/לוגיים שגויים מובהקים, כפל-תרגום בשמות, בחירת-מילה/צירוף שגויים מובהקים, ואי-התאמת מין/מספר חד-משמעית.
...
async function scopedFactFix(gameData: GameData, errors: FactError[]): Promise<string[]> {
  ...
  const instruction = `תקן אך ורק את השגיאות העובדתיות והלשוניות (בחירת-מילה שגויה / צירוף לא-תקני / אי-התאמת מין או מספר בין שם עצם לפועל/תואר) בשדות הטקסט של הסצנות הבאות. שמור על אותה משמעות, אורך, שפה (עברית) וסגנון — שנה רק את הטעות. **שמר על הניקוד אם קיים** (הוא תקין). אל תיגע במבנה, בחידות, בתשובות או במזהים.
...
async function runInputSafetyCheck(title: string, curriculum: string): Promise<SafetyVerdict> {
...
async function runOutputSafetyCheck(gameData: GameData): Promise<SafetyVerdict & { excerpt?: string }> {
...
async function logContentSafety(entry: {
  teacherId?: string
  questId?: string
  stage: 'input' | 'output'
  category?: string
...
const SAFETY_BLOCK_MESSAGE = 'לא ניתן היה ליצור הדמיה עבור הבקשה הזו. נסו לנסח את נושא ההדמיה או תוכן הלימוד באופן אחר.'
