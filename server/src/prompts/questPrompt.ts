...
3. CLIMAX SCENE — סצנת שיא (חובה! זו הנקודה הקריטית שנכשלת הכי הרבה):
   ...
   - הכניסה אל היעד (האקרופוליס, חדר הבקרה, ליבת התא, המעבדה הסודית) היא **סצנת השיא עצמה** — סצנה מלאה שמעמידה את רגע ההכרעה. **אסור** שהכניסה ליעד תהיה רק טריגר שמדלג ישר לסיום!
   ...
   - **זמן הפועל בנרטיב השיא כשיש בו חידה (קריטי — טעות חוזרת!):** נרטיב סצנת השיא מוצג לתלמיד **לפני** שהוא פותר את החידה שבה. לכן הוא חייב **להוביל אל** רגע פתירת החידה ולנסח את התוצאה כ**עתידית-מותנית**: "פתרו את החידה האחרונה — ואז הפורטל ייפתח / המשימה תושלם / הבוגד ייחשף", "רק אם תצליחו, X". **אסור לתאר בנרטיב הזה את הפתרון, הניצחון, פתיחת/סגירת הפורטל או השלמת המשימה כאילו כבר קרו** (לא "סיימנו!", לא "הפורטל נפתח לרווחה", לא "אתם עונים והכול מתייצב", לא "המשימה הושלמה", לא "הבלבול נגמר"). האחרית — מה שקרה **אחרי** ההצלחה — נמסרת אך ורק ב-endingGood/endingBad, לא בנרטיב השיא. (בסצנת שיא **ללא** חידה, הנרטיב כן מתאר את ההכרעה עצמה, כי אין צעד נוסף אחריו.)
