...
**2. שפה (חל על כל טקסט לתלמיד):** ${narrativeStyleSpec(level)} תמיד עברית **תקנית, עכשווית ובהירה** — לא ארכאית-מליצית, מילה יומיומית עדיפה תמיד על ספרותית תקינה ("שקטה" לא "שוקטת", "הסתכל" לא "הביט"), בדיוק מגדרי ומספרי, תחביר שלם (בלי נושא חסר). השפה משרתת את הרעיון; הקושי ברעיון, לא במילון — **הפרדה מוחלטת בין עומק (לא יורד) לניסוח (מותאם לרמה)**.
...
2. **שתי סצנות סיום** בשדות "endingGood" ו-"endingBad" ברמה העליונה של ה-JSON (לא בתוך scenes). מבנה כל סיום: { "title", "narrative", "drHoloDialog" (עברית), "imagePrompt" (אנגלית, **חייב** להכיל ${`{DR_HOLO}`} במרכז, **שונה מתמונת הפתיחה**), "drHoloExpression" (אנגלית, תואם לסוג הסיום) }:
   - "endingGood": חזרה למעבדה — הדוקטור נרגש וחולק את התובנה/מוסר ההשכל **העמוק** של החומר. תמונה: מעבדה חוגגת באור חם, אורות ניצחון הולוגרפיים, קריסטלים זוהרים במלואם; הבעה שמחה/גאה (למשל "a joyful, triumphant smile, beaming with pride").
   - "endingBad": חזרה למעבדה — הדוקטור חם ומעודד: מה הלך טוב, חסרים עוד קריסטלים לתובנה, מזמין לנסות שוב. **לעולם לא טון מאשים**. תמונה: מעבדה עמומה וקרה, אורות מהבהבים, קריסטלים דהויים; הבעה עייפה/מאוכזבת (למשל "a tired, dejected expression") — קודר אך **לא** מפחיד.
   - התייחסות לקריסטלים מותרת ורצויה בסיומים — שם זה חלק מהעלילה.`
}

/* ── קולו של ד"ר הולו: מהלכים פדגוגיים (roles), לא נוסחאות ──
   גרסה קודמת הזריקה ביטויי-חתימה כמחרוזות מדויקות מחייבות ("תנו לי להסביר את
   זה בעברית פשוטה") — הם חזרו זהים בכל הדמיה, נשמעו מתנשאים ושיבשו את זרימת
   הטקסט. העיקרון החדש: **שמור את התפקיד, בטל את הניסוח הקבוע** — המודל מממש
   כל מהלך בניסוח חופשי שמשתנה בין סצנות ובין הדמיות. כלל האצבע: ניסוח שמכריז
   על המהלך — אסור (עשה את המהלך); פנייה שמזמינה תגובת תלמיד — מותרת, בגיוון.
   הפניות-לדוגמה מוטות לצורת הפנייה (אסור לשון רבים בהדמיית יחיד). */
function drHoloVoiceInstructions(form: FormOfAddress): string {
  const ex =
    form === 'male'
      ? { imagine: 'דמיין ש…', act: 'מה היית עושה במקומו?', retell: 'איך היית מסביר את זה לחבר?' }
      : form === 'female'
        ? { imagine: 'דמייני ש…', act: 'מה היית עושה במקומה?', retell: 'איך היית מסבירה את זה לחברה?' }
        : { imagine: 'דמיינו ש…', act: 'מה אתם הייתם עושים?', retell: 'איך תסבירו את זה לכיתה?' }
  return `
## קולו של ד"ר הולו — מהלכים פדגוגיים (חשוב לאפקטיביות!)
בכל הדמיה צריכים להתקיים המהלכים הפדגוגיים הבאים ב-"drHoloDialog", אבל **בלי ניסוח קבוע** — נסח כל מהלך מחדש בכל הדמיה, במילים של הסצנה, משולב בזרימת הסיפור ולא כהכרזה נפרדת:
- **פישוט** — מיד אחרי מונח חדש/קשה: הסבר אותו ישירות במילים פשוטות, **בלי להכריז שאתה מפשט**.
- **הדמיה** — לפני מושג מופשט: תמונה מילולית קונקרטית (למשל "${ex.imagine}" — או כל פתיח אחר).
- **ניסוח-מחדש** — אחרי תובנה מרכזית: הזמנה לומר את זה במילים של התלמיד (למשל "${ex.retell}").
- **רלוונטיות** — קישור לחיי התלמיד דרך דוגמה מעולמו, **בלי להכריז** שזה רלוונטי.
- **הזדהות** — לפני דילמה/הכרעה: שאלה אמיתית (למשל "${ex.act}").
- **תמצות ועיגון זיכרון** — בשיא ובסיומים: מסקנה אחת חדה שנשארת — נאמרת ישירות, בלי פתיח-הכרזה.

כללים (קריטי):
- **כלל האצבע**: ניסוח שמכריז על המהלך — **אסור**; עשה את המהלך עצמו. אסורים במיוחד (קלישאות שחוקות): "תן/תני/תנו לי להסביר את זה בעברית פשוטה", "השורה התחתונה היא", "הנה למה זה חשוב לך/לכם", "הנה מה שאני רוצה שתזכור/שתזכרו". פנייה טבעית שמזמינה תגובה — מותרת, אך **גוון את ניסוחה**.
- אותו ניסוח-מעבר לא מופיע יותר מפעם אחת בהדמיה, והניסוחים משתנים גם בין הדמיות.
- 1-2 מהלכים לסצנה לכל היותר, בנקודה שבה הם באמת משרתים את ההסבר; כל מהלך הוא חלק ממשפט בעל תוכן, לא סיסמה.
- הכל **בדיוק בצורת הפנייה** שנקבעה למעלה.`
}
...
/* ── מגוון מסגורי פתיחה — שובר את תבנית "קריאת חירום" ──
   ...
   אחד ומזריק הנחיה מפורשת — גיוון דטרמיניסטי עדיף על בקשה כללית "היה מגוון". */
export const OPENING_FLAVORS = [
  'תעלומה: משהו לא מסתדר בנתונים או בממצאים — ד"ר הולו זקוק לעיניים חדות שיפענחו את החידה',
  'גילוי מרעיש: ממצא/שידור/אות חדש הרגע הגיע למעבדה — יוצאים לאמת אותו מקרוב',
  'תקלה במעבדה: הפורטל השתבש ופתח צוהר בלתי-צפוי לעולם התוכן — חייבים להיכנס ולהשיב את הסדר',
  'בקשת עזרה: דמות מתוך עולם התוכן (מדען, תושב, בעל תפקיד בן התקופה) פנתה לד"ר הולו ומבקשת עזרה דחופה',
  'חפץ מסתורי: חפץ/מסמך/דגימה הופיעו במעבדה ואיש אינו יודע את סודם — המסע יפענח אותו',
  'מרוץ נגד הזמן: אירוע חשוב עומד להתרחש בעולם התוכן, וחובה להשיג ידע מכריע לפני שיקרה',
  'שליחות הצלה: משהו או מישהו בעולם התוכן בסכנה, והידע הנלמד הוא הדרך היחידה להציל אותו',
  'אתגר מהדוקטור: ד"ר הולו הכין מסע מבחן מיוחד ומזמין את הצוות להוכיח את עצמו — עם הפתעות בדרך',
] as const

export function pickOpeningFlavor(rand: () => number = Math.random): string {
  return OPENING_FLAVORS[Math.floor(rand() * OPENING_FLAVORS.length)]
}

function openingFlavorBlock(): string {
  return `
## מסגור הפתיחה (הוגרל עבורך — חובה!)
פתח את ההרפתקה במסגור הבא: **${pickOpeningFlavor()}**
- בנה את סצנת המעבדה ואת המשימה המרכזית סביב המסגור הזה בדיוק (המשימה עדיין מוחשית ודחופה).
- **אסור להעתיק את סגנון הפתיחה של הדוגמה**: אל תשתמש בניסוח "קריאת חירום", וכותרת סצנת הפתיחה חייבת לשקף את המסגור שהוגרל — לא את הדוגמה.`
}
...
1. CENTRAL MISSION — משימה מרכזית (בפתיחה!): הסצנה הראשונה (מעבדת ד"ר הולו) חייבת לפתוח עם משימה ספציפית, מוחשית ודחופה — בעיה שדורשת פתרון עם סיבה לפעול **עכשיו**. **אסור** לפתוח ב"נלמד על X" / "היום נבקר ב-X" / "הגענו ל-X". הדוקטור מטיל משימה. דוגמאות:
   - היסטוריה: "אחד מחברי הבולה עומד להצביע נגד הרפורמה — עליך לגלות מי הוא ולמנוע את זה לפני פנות ערב"
...
3. CLIMAX SCENE — סצנת שיא (חובה! הנקודה שנכשלת הכי הרבה):
   - לפני סצנות הסיום חייבת **סצנה ייעודית** שבה המשימה המרכזית **נפתרת בפועל** — העימות/החשיפה/ההכרעה. הכניסה אל היעד (האקרופוליס, חדר הבקרה) היא סצנת השיא עצמה — **לא** טריגר שמדלג ישר לסיום.
   - היא לינארית: **ללא choices וללא nextSceneId** (היעדרו מסמן את המעבר ל-endingGood/endingBad). מומלץ לכלול בה חידה אחרונה מכריעה.
   - **זמן הפועל כשיש בה חידה (טעות חוזרת!):** הנרטיב מוצג לתלמיד **לפני** שפתר את החידה, ולכן מוביל אל רגע הפתירה ומנסח את התוצאה כ**עתידית-מותנית** ("פתרו את החידה — ואז הפורטל ייפתח / הבוגד ייחשף"). **אסור לתאר את הפתרון, הניצחון או השלמת המשימה כאילו כבר קרו** (לא "סיימנו!", לא "הפורטל נפתח לרווחה"). מה שקרה **אחרי** ההצלחה נמסר אך ורק ב-endingGood/endingBad. (בשיא **ללא** חידה — הנרטיב כן מתאר את ההכרעה עצמה.)
...
4ב. MOMENTUM — מתח שנמשך:
   - **hook בסוף כל סצנת ביניים**: המשפט האחרון של הנרטיב מוביל אל הסצנה הבאה — רמז שהתגלה, שאלה שנפתחה, קול/צל/תנועה שמושכים קדימה. לא "וכעת נמשיך הלאה".
   - **תפנית בשליש האחרון**: פרט אחד שמשנה את הבנת המשימה (בן-ברית לא-צפוי, הבעיה שונה משחשבו, רמז שהוביל למקום מפתיע) — מבוסס על התוכן הלימודי האמיתי, לא פנטזיה.
   - **כותרות כפרקי מתח**: כותרת כל סצנה מנוסחת כפרק בסיפור ("העקבות בבית המטבעה") — מגוונות זו מזו, לא תבנית אחת חוזרת.
...
**הדוגמה ממחישה מבנה בלבד — אסור להעתיק ממנה כותרות, ניסוחים או את מסגור הפתיחה שלה. ההרפתקה שלך נפתחת אך ורק במסגור שהוגרל עבורך (ראו סעיף "מסגור הפתיחה").**
{
  "scenes": [
    {
      "id": "scene_lab",
      ...
      "drHoloDialog": "האקלסיה היא פשוט האסיפה של כל האזרחים — שם, בהרמת יד, הוכרע גורל העיר כולה. אם מישהו זייף אסימונים, הוא גנב את ההחלטה מכל אתונה.",
    ...
    },
    {
      ...
      "drHoloDialog": "עצום רגע עיניים ותראה את זה: כיכר עמוסה, אלפי קולות, ואין אף מכונת הצבעה אחת. הכול נשען על אמון — ועל כך שכל אזרח יודע איך נראה אסימון אמיתי.",
      "puzzle": {
        "type": "multipleChoice",
    ...
    },
    {
      ...
      "drHoloDialog": "רגע של החלטה: להראות את החרס עכשיו, מול כולם — או לחכות ולאסוף עוד ראיות? מה היית בוחר, ולמה? לפעמים תזמון הוא ההבדל בין חשיפה לבין מבוכה.",
    ...
    },
    {
      ...
      "drHoloDialog": "דמוקרטיה איננה רק הזכות להצביע — היא המנגנון שמוודא שההצבעה נספרת ביושר. בלי זה, ההצבעה היא הצגה. את זה קח איתך הביתה."
    }
  ],
...
- **משלב יומיומי, לא ספרותי (קריטי!)**: כתוב בעברית שילד בגיל היעד **משתמש בה בעצמו** ומבין בקריאה ראשונה. אל תשתמש במילים ספרותיות או במשלב גבוה גם כשהן תקינות דקדוקית — "שקטה" ולא "שוקטת", "הסתכל" ולא "הביט", "התחיל" ולא "החל", "עמד" ולא "ניצב", "האלה" ולא "הללו", "לקח" ולא "נטל", "אמר" ולא "שח". המטרה היא בהירות, לא יופי ספרותי — עושר הסיפור מגיע מהתוכן, מהמתח ומהדמויות, לא מאוצר מילים מתוחכם. הכלל חל על **כל** הטקסט, ובמיוחד על תיאורי סצנה (שם הנטייה לפרוזה חזקה ביותר). זה **לא** אומר טקסט דל או ילדותי: משפטים חיים, דימויים מוחשיים ומתח סיפורי — רק במילים שילד באמת אומר. חריג: מונח מקצועי/תקופתי שנחוץ לתוכן הנלמד — השתמש בו והסבר אותו בהקשר; ההגבלה חלה על קישוט לשוני, לא על מונחי תוכן.
- **דיאלוג של דמויות היסטוריות (קריטי!)**: דמויות היסטוריות מדברות — זו סימולציה, ודיאלוג מדומיין רצוי. אבל הן מדברות **עברית תקנית ומובנת לתלמיד**, לא חיקוי ארכאי. **אסור** לייצר משלב "מקראי"/"עתיק" על חשבון תחביר ובהירות — כל משפט בדיאלוג חייב להיות תקין דקדוקית ומובן בקריאה ראשונה (לא "אחת אבן" — "אבן אחת"; לא "וולא יש הגנה" — "ואין הגנה"). אופי התקופה מועבר דרך **תוכן** הדברים (מה הדמות מספרת, מה חשוב לה, איך היא רואה את עולמה) — לא דרך תחביר עתיק או מילים ארכאיות. מונח תקופתי נחוץ (כלי, תפקיד, מקום) — השתמש בו נכון ובהקשר מובן, לא כקישוט. הדמות נשארת אישית וחיה — פשוט בעברית תקינה, באותה איכות שפה כמו ד"ר הולו. (ציטוט מקור מדויק ומוכר — כמו פסוק — מותר.)
...
- **אל תכפיל תרגום בשמות לועזיים**: אם השם הלועזי כבר מכיל את מילת-התפקיד (Abbey/River/Mount וכו') — אל תוסיף לפניו את המילה העברית המקבילה ("ווסטמינסטר אבי", לא "כנסיית ווסטמינסטר אביי"; "נהר התמזה", לא "נהר התמזה ריבר"). בספק — השם המקובל בעברית.
...
- **"drHoloDialog" בכל סצנה** (כשד"ר הולו פעיל) — קולו המלווה של הדוקטור: 1-3 משפטים שמפשטים, ממקדים או מעודדים, **בנוסף** לנרטיב ולא במקומו. שם מתממשים המהלכים הפדגוגיים (ראו "קולו של ד"ר הולו") — בניסוח חופשי, בלי נוסחאות קבועות. זהו הערוץ הפדגוגי החזק ביותר בהדמיה — אל תשמיט אותו.
...
3. **סצנת השיא (CLIMAX)**: הסצנה שאליה מובילה הבחירה הנעולה — בה המשימה נפתרת בפועל, לפי כללי CLIMAX SCENE שב-ADVENTURE NARRATIVE STRUCTURE (לינארית, ללא choices וללא nextSceneId; רק אחריה הסיום).
...
1. כל מנעול = **מכשול מוחשי ומפורש** בנרטיב (דלת נעולה, שומר שדורש הוכחה, דרך חסומה, מנגנון חסר-חלק, טרמינל שדורש סיסמה) — לא "בחירה נעולה" סתם. נרטיב סצנת המכשול מתאר אותו **ורומז מה נדרש כדי לעבור** ("השומר מצביע על חותם ריק על השער…"), כך שמי שמגיע בלי המפתח מבין מה לחפש.
2. המפתח מתאים למכשול **בהיגיון נרטיבי מלא**: שומר שדורש הוכחת אזרחות → אסימון הצבעה; מנגנון שבור → גלגל השיניים החסר. אסור "מגילה" שפותחת "מפולת סלעים" — חפץ בלי קשר הגיוני למכשול.
3. טקסט הענקת המפתח רומז לייעודו: "סוקרטס מוסר לך את המגילה: 'הראה אותה לשומר — הוא יידע שלמדת אצלי'".
4. לכל בחירה נעולה שדה "unlockText" — רגע הפתיחה בעברית, תואם למכשול ולמפתח ("השומר בוחן את המגילה ומחווה בראשו — הדרך פנויה").
...
- **הסצנה האחרונה היא סצנת השיא** לפי כללי CLIMAX SCENE שלמעלה — היחידה **ללא nextSceneId**. אסור שההדמיה תיגמר ב"הגענו ליעד" בלי רגע הכרעה.`
...
${params.questType === 'tour' ? '' : openingFlavorBlock()}
...
צור עכשיו את ה-JSON המלא של ההדמיה, לפי כל הכללים שב-system (דיוק עובדתי, הסברים, תמונות, שפה ופורמט הפלט) ולפי פרטי הקווסט שלמעלה.${params.questType === 'tour' ? '' : ' זכור: סצנת הפתיחה בנויה על המסגור שהוגרל עבורך בסעיף "מסגור הפתיחה" — אסור לפתוח ב"קריאת חירום".'} החזר JSON תקין בלבד.`
...
beat: ${climax.beat}
**כאן המשימה נפתרת** (העימות/החשיפה/ההכרעה). סצנה לינארית — **ללא nextSceneId** (היעדרו מסמן את המעבר לסיום).${ptype ? `
**זמן פועל (קריטי):** נרטיב השיא מוצג **לפני** שהתלמיד פותר את חידת השיא, ולכן חייב להוביל אל רגע הפתירה ולנסח את התוצאה כ**עתידית-מותנית** ("פתרו את החידה — ואז הפורטל ייפתח / המשימה תושלם"). **אסור** לתאר את הפתרון/הניצחון/פתיחת-הפורטל/השלמת-המשימה כאילו כבר קרו ("סיימנו!", "המשימה הושלמה", "אתם עונים והכול מתייצב") — האחרית נמסרת ב-endingGood/endingBad בלבד.` : ''}
${levelBlock(level)}
${niqqudBlock(level)}
${formOfAddressInstructions(params.formOfAddress ?? 'plural')}
