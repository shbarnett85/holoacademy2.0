...
- **רמה 10 (תקרה מוחלטת)**: מאתגר עד שגם תלמיד חזק מאוד לא עובר בקלות ולא תמיד מצליח.
...
/* בלוק אופי ההדמיה ובחירת התוכן לפי שכבת הגיל (סקאלת 1-20) — הרמה קובעת לא רק שפה
   אלא גם טון, רמת הפשטה, סוגי אתגרים ובחירת מושגים מתאימי-גיל. ידע המודל כברירת
   מחדל; תוכן המורה גובר אם סותר. */
function gradeCharacterBlock(level: number): string {
  const grade = levelToGradeLabel(level)
  const character =
    level <= 7
      ? 'עולם משחקי, מוחשי וקונקרטי — דמויות, חפצים וסיפור פשוט וברור, מושגים ניתנים-למישוש. הימנע מהפשטה מופשטת ומדילמות מורכבות.'
      : level <= 13
  ...
  return `
## אופי ההדמיה ובחירת התוכן לפי שכבת הגיל (קריטי!)
ההדמיה מיועדת לשכבת **${grade}** (רמה ${level}/20). הרמה קובעת לא רק את השפה אלא גם את **אופי ההדמיה ובחירת התוכן**:
- **אופי ההדמיה**: ${character}
- **בחירת התוכן**: בחר מושגים, דוגמאות ועומק **המתאימים למה שתלמיד ישראלי בשכבת ${grade} לומד ומבין** — לפי הידע הכללי שלך על שכבת הגיל.
- **המורה גובר**: אם "תוכן הלימוד" שהמורה סיפק מציין נושא/מושגים ספציפיים שסותרים את ברירת המחדל הגילית — **תוכן המורה מנצח**. עבוד לפי ברירת המחדל הגילית, אלא אם המורה ביקש במפורש אחרת.`
}
...
function niqqudBlock(level: number): string {
  if (level <= 6) {
    return `
## ניקוד מלא (קריטי — קוראים מתחילים!)
כל הטקסט המוצג לתלמיד (נרטיב, דיאלוג ד"ר הולו, שאלות, תשובות, הסברים, סיומים) חייב להיות **מנוקד במלואו** — ניקוד עברי תקני ומדויק על **כל** מילה (קמץ, פתח, חיריק, צירה, סגול, חולם, שורוק/קובוץ, שווא, ודגש לפי הצורך). הקוראים בשכבה זו (גן–ב') תלויים בניקוד.
**דייק תו-אחר-תו — ניקוד שגוי גרוע מהיעדר ניקוד (מלמד הגייה שגויה).** אם אינך בטוח בניקוד מילה מסוימת — נקד אותה בזהירות לפי הכללים, אך לעולם אל תמציא ניקוד שאינו תקני.`
  }
  return `
## ניקוד סלקטיבי (קריטי!)
...
export function buildQuestPrompt(params: QuestGenerationParams): string {
...
- **הסצנה האחרונה היא סצנת השיא** (ראו ADVENTURE NARRATIVE STRUCTURE): בה המשימה המרכזית נפתרת בפועל, והיא היחידה **ללא nextSceneId** (היעדרו מסמן את המעבר לסצנות הסיום). אסור שההדמיה תיגמר ב"הגענו ליעד" בלי רגע הכרעה.`
  ...
  return `אתה מעצב משחקי לומדה פדגוגיים בעברית עבור מערכת HoloAcademy.
