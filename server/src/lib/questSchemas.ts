...
const KNOWN_ERROR_PATTERNS: { re: RegExp; problem: string; correction: string }[] = [
  ...
  /* ביטויי-הכרזה קלישאתיים של ד"ר הולו (הנוסחאות הקבועות שבוטלו): המהלך הפדגוגי
     נשמר, ההכרזה עליו אסורה — עשה את המהלך ישירות. */
  { re: /לי להסביר את זה בעברית פשוטה/, problem: 'ביטוי-הכרזה קלישאתי ("תן/תני/תנו לי להסביר את זה בעברית פשוטה") — אסור להכריז על פישוט', correction: 'מחק את ההכרזה והסבר ישירות במילים פשוטות (שמור את תוכן ההסבר עצמו)' },
  { re: /השורה התחתונה היא/, problem: 'ביטוי-הכרזה קלישאתי ("השורה התחתונה היא") — אסור להכריז על תמצות', correction: 'מחק את הפתיח ואמור את המסקנה ישירות' },
  { re: /הנה למה זה חשוב לכ?ם?(?![א-ת])|הנה למה זה חשוב לך/, problem: 'ביטוי-הכרזה קלישאתי ("הנה למה זה חשוב לך/לכם") — אסור להכריז על רלוונטיות', correction: 'מחק את ההכרזה והראה את הרלוונטיות דרך דוגמה מחיי התלמיד' },
  { re: /הנה מה שאני רוצה שתזכ/, problem: 'ביטוי-הכרזה קלישאתי ("הנה מה שאני רוצה שתזכרו") — אסור להכריז על עיגון-זיכרון', correction: 'מחק את הפתיח ואמור את הנקודה שצריכה להישאר, ישירות' },
]
