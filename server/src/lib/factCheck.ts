...
import { extractJson, checkAnswerConsistency, lintDetachedClitics, lintHighRegister, lintKnownErrors, lintRepetition, type FactCheckMeta, type GameData } from './questSchemas.js'
...
5. **התאמות דקדוקיות שגויות** — (א) **מין**: שם עצם נקבה/זכר עם פועל-תואר-מילת-שאלה בגוף הלא-נכון (למשל "כמה שוקל הגלקסיה"→"שוקלת", "איזו אור"→"איזה", "המים זורמת"→"זורמים"), במיוחד בשמות עצם לא-אינטואיטיביים (שמיים=זכר רבים, דלת=נקבה, שולחן=זכר). (ב) **מספר**: יחיד/רבים (למשל "הילדים הולך"→"הולכים"), במיוחד שמות עצם שנראים יחיד אך הם רבים (מים, שמיים, חיים) ולהפך. (ג) **זמן פועל**: קפיצה לא-מוסברת בין עבר להווה בתוך אותה פסקה/משפט. (ד) **סמיכות מאולצת**: "לב ביסקוויט" במקום "הלב של ביסקוויט" — סמיכות ספרותית לא-טבעית לילדים. (ה0) **סיומת חבורה תלושה** — סיומת שנכתבה כמילה נפרדת ("ליד כם"→"לידכם", "אצל כם"→"אצלכם"); "כם" איננה מילה עברית לעולם — דגל תמיד. (ה) **צורת פועל מומצאת/שגויה**, במיוחד בגוף שני רבים בעבר — למשל "תודיתם" (לא מילה; הנכון "הודיתם" מהפועל להודות), "ברכתם" שמנוקד כשם עצם. (ו) **גזירה מורפולוגית שגויה — שורש נכון, משקל/בניין שגוי**: מילה מהשורש הנכון אך בצורה של דבר אחר, ששתי הצורות בה מילים תקינות במילון (בודק איות לא יתפוס) — הבדיקה היא אם הצורה מתאימה לכוונה בהקשר. במיוחד: **אדם↔כלי** ("מחשבים כמו פרידריך האייק" → **הוגים/חושבים**; "מחשבים"=מכונות), **אקטיבי↔פסיבי** ("פקידים שלא בחרו אנשים" כשהכוונה שהאנשים לא בחרו אותם → "פקידים **שלא נבחרו**"), פועל↔שם-עצם. דגל רק אי-התאמה **חד-משמעית ובטוחה** (לא שם עצם דו-מיני/צורת-רבים חריגה שנויה-במחלוקת).
6. **משפטים שבורים או חסרי נושא** — משפט שנראה כתוצר של דחיסה/עריכה חלקית: חסר נושא ברור, פועל תלוי-באוויר, או צירוף מילים שלא מרכיב משפט תקין דקדוקית. אם אי-אפשר לזהות "מי עושה מה" בקריאה ראשונה — דגל, **גם כשכל מילה בו לבדה תקינה** (למשל "ידעתי שאל תהיה נגדי נגיעה בטרם שחרר הידית" — לא מובן; נסח מחדש את הכוונה בעברית פשוטה). **חל במלואו גם על דיאלוג מצוטט של דמויות (טקסט בתוך מירכאות)**: חיקוי "מקראי"/ארכאי עם תחביר שבור איננו סגנון לגיטימי אלא שגיאה (למשל "אחת אבן"→"אבן אחת", "וולא יש הגנה"→"ואין הגנה", משפט פסאודו-עתיק שאינו מובן בקריאה ראשונה). ב-correction נסח מחדש בעברית תקנית תוך שמירת התוכן והדובר. (ציטוט מקור אמיתי ומדויק — כמו פסוק תנ"כי — תקין ואינו מדוגל.)
7. **בחירת-מילה שגויה סמנטית וצירוף מתורגם-מילולית** — (א) פועל/שם-עצם שלא מתאים סמנטית להקשר גם אם דקדוקית תקין (למשל "דגם לו לאיזה כיוון ללכת" — "דגם"=יצר-דגם, הכוונה "הצביע/רמז"). (ב0) **ניב/צירוף-כבול מרוסק** — ניב שהורכב לא נכון ("מן הסוף לסוף"→"מההתחלה ועד הסוף", "יד ביד עם"→"יחד עם") — דגל תמיד; (ב) צירוף מתורגם-מילולית מאנגלית שאינו עברית טבעית (למשל "סביב אתכם"=around you, התקני "סְבִיבְכֶם"). (ג) עברית "מתורגמת"/גבוהה-מדי שמרגישה מאולצת בפי ילד — העדף "ה-X של Y" על סמיכות ספרותית ומשפטים קצרים (עד כ-15 מילה) על פסוקיות מרובות. **התעלם מהניקוד** — הוא תקין; בדוק בחירת-מילה ותחביר בלבד.
...
${briefBlock}
אל תדגל סגנון, ניסוח ספרותי תקין, העדפה אישית או מילים נדירות-אך-תקינות — רק שגיאות מהסוגים למעלה, חד-משמעיות ובטוחות. **הפטור הסגנוני אינו חל על תחביר שבור**: "סגנון תקופתי" בדיאלוג אינו מצדיק משפט לא-דקדוקי או לא-מובן (ראו סוג 6).
...
export async function factCheckInBackground(questId: string, gameData: GameData, baseWarnings: string[], level: number, form: FormOfAddress, factBrief?: string): Promise<void> {
  ...
  try {
    ...
    /* לינטים דטרמיניסטיים נוספים: שגיאות-ידועות (רשימה מצטברת) + חזרתיות
       חוצת-סצנות (הבודק-AI לא משווה בין סצנות). נכנסים לאותו צינור תיקון. */
    const extraLint = [...lintKnownErrors(gameData), ...lintRepetition(gameData)]
    if (extraLint.length) {
      info(`[fact-check] lint: ${extraLint.length} שגיאות-ידועות/חזרתיות`)
      /* dedup מול דיגולי-AI על אותה בעיה באותה סצנה (למשל וולא שנתפס גם וגם) */
      fc.errors = [...fc.errors, ...extraLint.filter((l) => !fc.errors.some((e) => e.sceneId === l.sceneId && l.problem.slice(0, 20) === e.problem.slice(0, 20)))]
      fc.ok = true
    }
