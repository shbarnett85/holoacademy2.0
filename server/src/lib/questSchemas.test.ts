...
describe('lintKnownErrors — ביטויי-הכרזה קלישאתיים (הנוסחאות שבוטלו)', () => {
  const gdDialog = (drHoloDialog: string): GameData =>
    ({ scenes: [{ id: 's1', title: 'בדיקה', drHoloDialog }], entrySceneId: 's1' }) as unknown as GameData

  it('כל ארבעת ביטויי-ההכרזה מדוגלים, בכל הטיה', () => {
    for (const bad of [
      'תן לי להסביר את זה בעברית פשוטה: המקדש היה מרכז.',
      'תני לי להסביר את זה בעברית פשוטה: הכוורת חיה.',
      'השורה התחתונה היא שהמסחר ניצח.',
      'הנה למה זה חשוב לכם: גם היום מצביעים.',
      'הנה למה זה חשוב לך: גם היום מצביעים.',
      'הנה מה שאני רוצה שתזכרו: אמון הוא הבסיס.',
      'הנה מה שאני רוצה שתזכור: אמון הוא הבסיס.',
    ]) {
      expect(lintKnownErrors(gdDialog(bad)).some((h) => h.problem.includes('הכרזה')), bad).toBe(true)
    }
  })

  it('פניות מזמינות-תגובה ומהלכים ישירים — לא מדוגלים', () => {
    for (const ok of [
      'מה אתם הייתם עושים במקומו? לפעמים אין בחירה קלה.',
      'דמיינו רגע: כיכר עמוסה באלפי אנשים.',
      'האקלסיה היא פשוט האסיפה של כל האזרחים.',
      'איך תסבירו את זה לכיתה? נסו במילים שלכם.',
      'זה חשוב לכם כי גם היום, בבחירות לכיתה, כל קול נספר.',
    ]) {
      expect(lintKnownErrors(gdDialog(ok)), ok).toHaveLength(0)
    }
  })
})
